<?php
	...
	$query = "SELECT codigo_orden, descripcion FROM orden WHERE codigo_clase = '$codigo_clase' ORDER BY descripcion";
	$resultado=$mysqli->query($query);
	$html= "<option value='0'>Seleccione orden </option>";
	while($row = $resultado->fetch_assoc())
	{
		$html.= "<option value='".$row['codigo_orden']."'>".$row['descripcion']."</option>";
	}
	echo $html;
?>
<?php
